...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (isDoctor && userInfo) {
      fetch(
        `https://api-doctors-portal.onrender.com/allPatients?email=${userInfo.email}`,
        {
          method: "GET",
          headers: {
        ...
        .then((res) => res.json())
        .then((data) => {
          const latestAppoint = data.filter(
            (appointment) => appointment.appointmentDate === latestDate
          );
